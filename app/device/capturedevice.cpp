...
void CaptureDevice::removeSelfmixedSignal(SelfmixedSignal* s)
{
    if (mSelfmixedSignalList.contains(s)) {
        mSelfmixedSignalList.removeOne(s);
        delete s;

        reconfigure(); // reconfiguraiton
    }
}


QList<int> CaptureDevice::unusedSelfmixedIds()
{
    QList<int> l;
    for (int i = 0; i < maxNumSelfmixedSignals(); i++) {
        bool used = false;
        foreach(SelfmixedSignal* s, mSelfmixedSignalList) {
            if (s->id() == i) {
                used = true;
                break;
            }
